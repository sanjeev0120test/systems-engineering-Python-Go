# Windows check helper — mirrors check.sh for steps run from PowerShell.
param(
...
function Pass($n) {
    Write-Host ""
    Write-Host "PASS: Step $n complete" -ForegroundColor Green
    exit 0
}
...
$stepMap = @{
    "0"  = { & $Python -c "import pytest,yaml,requests,fastapi; from python.common.logging_setup import new_trace_id; from python.common.paths import sample_data_path; assert sample_data_path('logs','access.log').exists()"; Pass 0 }
    "1"  = { & $Python -m pytest python/01_python_basics/step_01_variables/test_lesson.py -q; if ($LASTEXITCODE -eq 0) { Pass 1 } else { Fail 1 } }
    "7"  = { & $Python -m pytest python/02_file_handling/test_lesson.py -q; if ($LASTEXITCODE -eq 0) { Pass 7 } else { Fail 7 } }
    "9"  = { & $Python -m pytest python/04_logging/test_lesson.py -q; if ($LASTEXITCODE -eq 0) { Pass 9 } else { Fail 9 } }
    "16" = { & $Python -m pytest python/13_health_checker/test_lesson.py -q; if ($LASTEXITCODE -eq 0) { Pass 16 } else { Fail 16 } }
    "all" = {
        & $Python -m pytest python/ -q
        if ($LASTEXITCODE -ne 0) { Fail $Step }
        Write-Host "ALL PYTHON TESTS PASS (use WSL ./check.sh for Go steps 32-36)" -ForegroundColor Green
        exit 0
    }
}
...
if ($stepMap.ContainsKey($Step)) {
    ...
    if ($paths.ContainsKey($Step)) {
        & $Python -m pytest $paths[$Step] -q
        if ($LASTEXITCODE -eq 0) { Pass $Step } else { Fail $Step }
    } else {
        Write-Host "For Go steps 32-36 use WSL: wsl ./check.sh $Step"
        exit 1
    }
}
